...
# سه دسته محتوا: انیمه، مانهوا، مانگا
RSS_FEEDS = {
    "Anime News Network": {
...
def format_message(item):
    """قالب‌بندی یکسان برای همه دسته‌ها"""
...
def main():
    print("=" * 50)
    print(f"شروع اجرا - {datetime.now(timezone.utc).isoformat()}")
    print("=" * 50)
    ...
    print(f"تعداد اخبار قبلی: {len(posted_ids)}")
    ...
    # شمارش بر اساس دسته
    anime_count = sum(1 for i in all_items if i["category"] == "anime")
    manga_count = sum(1 for i in all_items if i["category"] == "manga")
    manhwa_count = sum(1 for i in all_items if i["category"] == "manhwa")
    print(f"🎌 انیمه: {anime_count} | 📚 مانگا: {manga_count} | 📖 مانهوا: {manhwa_count}")
    
    new_items = [item for item in all_items if item["id"] not in posted_ids]
    print(f"اخبار جدید: {len(new_items)}")
    ...
    print(f"\n{'=' * 50}")
    print(f"پایان اجرا - {posted_count} خبر ارسال شد")
    print(f"{'=' * 50}")
...
def translate_to_persian(text):
    """ترجمه متن به فارسی با استفاده از MyMemory API (رایگان)"""
    if not text or len(text.strip()) < 3:
        return text
    try:
        # محدودیت API: حداکثر 500 کاراکتر در هر درخواست
        if len(text) > 450:
            text = text[:450] + "..."
        ...
        params = {
            "q": text,
            "langpair": "en|fa",
            "de": "animebot@example.com"  # ایمیل برای افزایش محدودیت
        }
        res = requests.get(url, params=params, timeout=10)
        if res.status_code == 200:
            data = res.json()
    ...
    for source_name, config in RSS_FEEDS.items():
        try:
            ...
            for entry in feed.entries[:15]:
                ...
                time.sleep(0.5)  # جلوگیری از محدودیت API ترجمه
        except Exception as e:
            print(f"خطا در دریافت {source_name}: {e}")
    return all_news
...
def format_manhwa_message(item):
    ...
    lines.append(f"📂 📖 مانهوا")
    lines.append(f"📌 منبع: MangaDex")
